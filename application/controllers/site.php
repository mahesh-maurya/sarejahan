...
//			$data['table']=$this->navigation_model->viewnavigation();
    ...
    public function createwalloffamesubmit() 
    {
        ...
        $this->form_validation->set_rules("status","Status","trim");
        $this->form_validation->set_rules("name","Name","trim");
        $this->form_validation->set_rules("fromemail","fromemail","trim");
        $this->form_validation->set_rules("toemail","toemail","trim");
        $this->form_validation->set_rules("date","date","trim");
        $this->form_validation->set_rules("place","place","trim");
        $this->form_validation->set_rules("shortdeed","shortdeed","trim");
        ...
        else
        {
            ...
            if($this->walloffame_model->create($name,$deed,$status,$fromemail,$toemail,$date,$place,$shortdeed,$image)==0)
    ...
    public function editwalloffamesubmit()
    {
        ...
        else
        {
            ...
            if($image=="")
            {
            $image=$this->walloffame_model->getwalloffameimagebyid($id);
               // print_r($image);
                $image=$image->image;
            }
            
            
            if($this->walloffame_model->edit($id,$name,$deed,$status,$fromemail,$toemail,$date,$place,$shortdeed,$image)==0)
